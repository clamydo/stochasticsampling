...
fn test_polarization_from_distribution() {
    ...
    assert_eq!(p.field[[0, 0, 0, 0]].re, -1.0_f32 / n);
    assert_eq!((p.field[[1, 0, 0, 0]].re * 10e14).round(), 0.0_f32 / n);
    assert_eq!((p.field[[2, 0, 0, 0]].re * 10e14).round(), 0.0_f32 / n);

    // set singular entry to zero
    p.field
        .slice_mut(s![.., 0, 0, 0])
        .map_inplace(|v| *v = 0.0_f32.into());
