...
///
fn get_k_sampling(grid_size: GridSize, box_size: BoxSize) -> Vec<Array<Complex<f32>, Ix1>> {
    let ks: Vec<Array<Complex<f32>, Ix1>> = [grid_size.x, grid_size.y, grid_size.z]
        .iter()
        ...
        .map(|(gs, bs)| {
            ...
            let values: Array<Complex<f32>, Ix1> = Array::from_vec(
                (-(a as i64)..(b as i64))
                    .map(|i| Complex::new((i as f32) * step, 0.))
                    .collect(),
            );
...
pub fn get_k_mesh(grid_size: GridSize, box_size: BoxSize) -> Array<Complex<f32>, Ix4> {
    let ks = get_k_sampling(grid_size, box_size);
    mesh3d::<Complex<f32>>(&ks)
}
...
/// The first axis denotes the components of the k-vector:
///     `res[c, i, j, m] -> k_c[i, j, m]`
pub fn get_norm_k_mesh(grid_size: GridSize, box_size: BoxSize) -> Array<Complex<f32>, Ix4> {
    let ks = get_k_sampling(grid_size, box_size);
    let mesh = mesh3d::<Complex<f32>>(&ks);
...
pub fn get_inverse_norm_squared(k_mesh: ArrayView<Complex<f32>, Ix4>) -> Array<Complex<f32>, Ix3> {
...
///
/// The inverse norm of k=0 is set to zero, i.e. 1/(k=0)^2 == 0
pub fn get_inverse_norm(k_mesh: ArrayView<Complex<f32>, Ix4>) -> Array<Complex<f32>, Ix3> {
    let squared = &k_mesh * &k_mesh;
