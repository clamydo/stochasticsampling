...
/// components.
#[derive(Debug)]
pub struct FFTData2DMatrix<'a> {
    pub data: ArrayViewMut<'a, Complex<f64>, (Ix, Ix, Ix, Ix)>,
}


impl<'a> FFTData2DMatrix<'a> {
    /// Return a new FFTData2 instance. Only considers the first
    pub fn new(shape: (Ix, Ix, Ix, Ix)) -> FFTData2DMatrix<'a> {
        let data;
        unsafe {
            let ptrin = ::fftw3_ffi::fftw_malloc(shape.0 * shape.1 * shape.2 * shape.3 *
                                                 mem::size_of::<Complex<f64>>());
            data = ArrayViewMut::from_shape_ptr(shape, ptrin as *mut Complex<f64>);
        }

        FFTData2DMatrix { data: data }
    }
}

/// Automatically free memory, if goes out of scope.
impl<'a> Drop for FFTData2DMatrix<'a> {
    fn drop(&mut self) {
        unsafe { ::fftw3_ffi::fftw_free(self.data.as_ptr() as *mut _) }
    }
}
