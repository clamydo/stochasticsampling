...
pub struct FFTData2D<'a> {
    pub data: ArrayViewMut<'a, Complex<f32>, Ix2>,
}
...
impl<'a> FFTData2D<'a> {
    ...
    pub fn new(shape: Ix2) -> FFTData2D<'a> {
        ...
        unsafe {
            let ptr =
                ::fftw3_ffi::fftwf_malloc(shape[0] * shape[1] * mem::size_of::<Complex<f32>>());
            data = ArrayViewMut::from_shape_ptr(shape, ptr as *mut Complex<f32>);
        }
...
impl<'a> Drop for FFTData2D<'a> {
    fn drop(&mut self) {
        unsafe { ::fftw3_ffi::fftwf_free(self.data.as_ptr() as *mut _) }
    }
}
...
pub struct FFTData3D<'a> {
    pub data: ArrayViewMut<'a, Complex<f32>, Ix3>,
}
...
impl<'a> FFTData3D<'a> {
    ...
    pub fn new(shape: Ix3) -> FFTData3D<'a> {
        let data;
        unsafe {
            let ptr = ::fftw3_ffi::fftwf_malloc(
                shape[0] * shape[1] * shape[2] * mem::size_of::<Complex<f32>>(),
            );
            data = ArrayViewMut::from_shape_ptr(shape, ptr as *mut Complex<f32>);
        }
